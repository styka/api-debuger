...
app.controller('Operation', function($scope, $http, $resource) {
	...
	$scope.loadGenerateOperation = function() {
		$http.get(getUrl('/generateOperation')).success(function(generateOperation) {
...
//		$http.get(getUrl('/operation/'+id)).success(function(operation) {
//			$scope.operation = operation;
//			$scope.refreshActionButtons();
//			$scope.showOperationActions();
//		});
//	}
	...
	$scope.createOperation = function(operation) {
		var operationResource = $resource(getUrl('/operation'));
		operation.id = null;
	...
	$scope.updateOperation = function(operation) {
		var operationResource = $resource(getUrl('/operation/'+operation.id),null,{
		    'update': { method:'PUT' }
		});
	...
	$scope.deleteOperation = function(id){
		$http.delete(getUrl('/operation/'+id)).success(function() {
